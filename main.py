...
def detect_in_area(t1,t2,points):
...
def deal_area_result(points,sub_amount):
    def gen_args(len_,sub_amount):
        parts = []
        for i in range(len_//sub_amount):
            parts.append((i*sub_amount,(i+1)*sub_amount,points))
        parts.append((len_-len_%sub_amount,len_,points))
        return parts
...
def detect_fire_by_torch():
    ...
    result = fire_predictor.predict_bypath(frame_folder[:-3])
    ...
    start = False
    filed_result = []
    destination_path = 'data/output_frames/fired/'
...
def detect_helmet(t1,t2):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    predictor = Predictor(device=device)
    result_boxs = []
    nohelmet_path = 'data/output_frames/no_helmet/'
...
def deal_helmet_result(sub_amount):
    def gen_args(len_,sub_amount):
        parts = []
        for i in range(len_//sub_amount):
            parts.append((i*sub_amount,(i+1)*sub_amount))
        parts.append((len_-len_%sub_amount,len_))
        return parts
...
def main_fire(mg_dic):
    ...
    names_fired = detect_fire_by_torch()
    paths_fired_remote = [minio_root+'detection/fired/'+str(datetime.date.today())+'/'+file_name for file_name in names_fired]
    paths_fired_local = ["data/output_frames/fired/"+file_name for file_name in names_fired]
...
def main_helmet(sub_amount,mg_dic):
    ...
    minio_db = net_tool.MyMinio('zhgd')
    names_helmet = deal_helmet_result(sub_amount)
    paths_helmet_remote = ['detection/no_helmet/'+str(datetime.date.today())+'/'+file_name for file_name in names_helmet]
    paths_helmet_local = ["data/output_frames/no_helmet/"+file_name for file_name in names_helmet]
...
def main_area(points,sub_amount,mg_dic):
    ...
    if len(points)==8:
        minio_db = net_tool.MyMinio('zhgd')
        names_area = deal_area_result(points,sub_amount)
        paths_area_remote = ['detection/in_area/'+str(datetime.date.today())+'/'+file_name for file_name in names_area]
        paths_area_local = ["data/output_frames/in_area/"+file_name for file_name in names_area]
...
def main(url:str,points:List,frame_interval,sub_amount):
    ...
    dir_tools.clear_dir('data/output_frames')
    ...
    with mp.Manager() as mg:
        ...
        results = { 'eara':ocr_tool.get_video_eara('data/output_frames/frames/1/frame_0.jpg'),
                    'fired':mg_dic['fired'],
...
if __name__=='__main__':
    ...
    # url = 'http://10.83.190.141:7000/file/output_2024-05-16_11-54.mp4'
